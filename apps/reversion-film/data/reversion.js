// Researched values for the film, ported verbatim from the scrolly story's
// js/04-data.js (new-stories/reversion-to-mean/). Only the module wrapper
// changed: an IIFE assigning RTM.data became named ES exports.
//
// THE UNIT OF THE WHOLE PIECE IS SIGMA. Every person is placed as a z score
// above the mean of a NAMED reference population, and each pair states which
// of three constructions produced it in `basis`:
//   A. Moment z.  z = (stat - mean) / sd inside a named population.
...
//                 measurement, and the film says so on the figure.
//
// Cross-domain z scores are not strictly interchangeable. Within a pair the
// parent and child always share one population and one construction, which is
// what the regression slope actually rests on. Provenance for every number is
// in new-stories/reversion-to-mean/reversion/RESEARCH.md. Do not retype a
// value here; regenerate from the story file.
...
const NBA_POP = "NBA players with 200 or more career games. Career points per game, mean 9.0, sd 4.8.";
const MLB_POP = "MLB position players with 3,000 or more career plate appearances. Career OPS+, mean 100, sd 17.";
const NHL_POP = "NHL forwards with 400 or more career games. Career goals per game, mean 0.24, sd 0.11.";
const NFL_POP = "Everyone who has played an NFL regular season game, about 27,000 men, ranked by career Approximate Value.";
const GOLF_POP = "Golfers with 50 or more PGA Tour starts since 1950, about 3,000 men, ranked by career Tour wins.";
const FUT_POP = "Players with 100 or more appearances in a top five European league since 1960, about 12,000 men, ranked by career standing.";
const OFFICE_POP = "Adults of the country, ranked by the highest office reached. See the rubric.";
...
// The height correlation is read from the trait row rather than typed, so the
// film cannot quote two different values for one constant the way the story
// once did (0.45 in two scenes, 0.47 in two others).
function traitR(id, fallback) {
  const t = TRAITS.find((x) => x.id === id);
  return t && isFinite(t.r) ? t.r : fallback;
}

export const HEIGHT = { mean: 69.1, sd: 3.0, r: traitR("height", 0.47), lebron: 79.25, bronny: 73.5 };

// Generations for an advantage to halve, given a parent-child correlation.
export function halfLife(r) { return Math.log(0.5) / Math.log(r); }

// A `source` field anywhere above is an ID into SOURCES, never text a reader
// should see. Fall back to the key rather than to nothing, so a broken id is
// visible in review instead of silently blank.
export function cite(id) {
  if (!id) return "";
  const s = SOURCES.find((x) => x.id === id);
  return s ? s.cite : String(id);
}
...
export function byDomain(d) { return DYNASTIES.filter((x) => x.domain === d); }
